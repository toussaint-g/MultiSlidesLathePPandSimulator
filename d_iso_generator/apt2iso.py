...
class IsoWriter:
    ...
    def __init__(self, machine_config: JsonDict, channel_name: str) -> None:
        ...
        self.out: list[str] = []
        # Ces caches evitent de remettre feedrate et spindle dans l'ISO si ils n'ont pas change, pour rendre le code plus lisible.
        self.last_x_position: Optional[float] = None
        self.last_y_position: Optional[float] = None
        self.last_z_position: Optional[float] = None
        self.last_feedrate_value: Optional[float] = None 
        self.last_feedrate_unit: Optional[FeedrateUnit] = None
        self.last_spindle_speed: Optional[float] = None
        self.last_spindle_direction: Optional[SpindleDirection] = None

        try:
            self.machine = MachineParameters.from_config(machine_config, channel_name, home_x_mode="machine")
            self.last_work_plane_code: Optional[str] = None
        except KeyError:
    ...
    def _get_spindle_start_for_tool(self, spindle_direction: SpindleDirection, tool_number: int) -> str:
...
# En cours: spindle_on et spindle_off
    ...
    # TODO: Verifier le sens de rotation des broches de tournage
    def spindle_start(self, tool_number: int, spindle_speed: float, spindle_unit: SpindleUnit, spindle_direction: SpindleDirection) -> None:
        ...
        if self.last_spindle_speed != spindle_speed or self.last_spindle_direction != spindle_direction:
            self.emit(f"{self._get_spindle_start_for_tool(spindle_direction, tool_number)} {self.machine.spindle_speed_prefix}{format_float_to_iso(spindle_speed)}")
            self.last_spindle_speed = spindle_speed
            self.last_spindle_direction = spindle_direction
    ...
    def spindle_stop(self, tool_number: int) -> None:
        ...
        if tool_config is None:
            raise ValueError(f"MachineConfigError: outil {tool_number} introuvable dans le canal {self.machine.channel_name}")
        self.emit(f"{format_float_to_iso(tool_config.get("spindlestop"))}")
    ...
    def tool_change(self, tool_number: int) -> None:
        """Gere le changement d'outil en emettant la ligne de changement d'outil correspondante."""
        if tool_number < 10:
            tool_number_with_corrector = f"0{tool_number}"
        else:
            tool_number_with_corrector = str(tool_number)
        self.emit(f"{self.machine.toolname_prefix}{tool_number_with_corrector}{tool_number_with_corrector}")
    ...
    def circular_move(self, work_plane: str, motion_code: str, feedrate_value: float, feedrate_unit: Optional[FeedrateUnit],
        ...
        if self.last_work_plane_code != work_plane:
            axis_words.append(work_plane)
            self.last_work_plane_code = work_plane

        axis_words.append(motion_code)

        start_x = self.last_x_position if self.last_x_position is not None else self.machine.home_tool_x
        start_y = self.last_y_position if self.last_y_position is not None else self.machine.home_tool_y
        start_z = self.last_z_position if self.last_z_position is not None else self.machine.home_tool_z
...
Handler = Callable[[str, str, MachineState, IsoWriter], None]


def h_comment(apt_keyword: str, argument_text: str, state: MachineState, iso_writer: IsoWriter, text_info: str | None = None) -> None:
...
def h_channel(apt_keyword: str, argument_text: str, state: MachineState, iso_writer: IsoWriter) -> None:
...
def h_op_name(apt_keyword: str, argument_text: str, state: MachineState, iso_writer: IsoWriter) -> None:
...
def h_spindle(apt_keyword: str, argument_text: str, state: MachineState, iso_writer: IsoWriter) -> None:
    ...
    spindle_unit = SpindleUnit(spindle_tokens[1].upper())
    spindle_direction = SpindleDirection(spindle_tokens[2].upper())
...
def h_loadtl(apt_keyword: str, argument_text: str, state: MachineState, iso_writer: IsoWriter) -> None:
...
def h_tdata(apt_keyword: str, argument_text: str, state: MachineState, iso_writer: IsoWriter) -> None:
...
def h_rapid(apt_keyword: str, argument_text: str, state: MachineState, iso_writer: IsoWriter) -> None:
    """Le prochain GOTO utilisera G0."""
    state.motion_mode = MotionMode.RAPID

def h_feedrat(apt_keyword: str, argument_text: str, state: MachineState, iso_writer: IsoWriter) -> None:
    ...
    feedrate_unit = FeedrateUnit(feedrate_tokens[1].upper())
    # Si l'unite d'avance a change, on l'ajoute a la ligne de mouvement.
...
def h_goto(apt_keyword: str, argument_text: str, state: MachineState, iso_writer: IsoWriter) -> None:
...
def h_indirv(apt_keyword: str, argument_text: str, state: MachineState, iso_writer: IsoWriter) -> None:
...
def h_tlon(apt_keyword: str, argument_text: str, state: MachineState, iso_writer: IsoWriter) -> None:
...
def h_fini(apt_keyword: str, argument_text: str, state: MachineState, iso_writer: IsoWriter) -> None:
    """Termine le programme ISO."""
    iso_writer.footer()


def h_default(apt_keyword: str, argument_text: str, state: MachineState, iso_writer: IsoWriter) -> None:
